...
`System > MIDI Panic` and every X68000 reset path send All Sound Off and All Notes Off on all 16 channels, plus explicit Note Off messages for all 128 keys on every channel, to the hosted Audio Unit, CoreMIDI OUT-A, and RS-232C OUT-B. `System > GM Reset` separately sends the GM System On reset SysEx to those same destinations; neither operation uses the normal physical-output delay, and pending delayed serial events are discarded before either sequence is sent.
